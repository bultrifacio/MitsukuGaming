    ...
    public static void backup () throws NamingException {
        ...
        try (PrintWriter writer = new PrintWriter(System.getProperty("user.home") + "/Desktop/BackupSales.txt", "UTF-8")) {
                ...
                for (Sales sale : salesList) {
                    writer.println("INSERT INTO APP.SALES (PRODUCT_ID, USER_ID, \"DATE\", \"METHOD\") VALUES ("+ sale.getProductId() +", "+ sale.getUserId() +", '"+sale.getDate().getYear()+"-"+ sale.getDate().getMonth()+"-"+ sale.getDate().getDay()+"', '"+ sale.getMethod() +"');");
                }
            } catch (FileNotFoundException | UnsupportedEncodingException ex) {
            Logger.getLogger(Backup.class.getName()).log(Level.SEVERE, null, ex);
        }*/
